...
def setup_ui(app, img_width=1200, img_height=900):
    ...
    app.root.geometry(f"{img_width+100}x{img_height+350}")
    ...
    ttk.Button(nav_frame, text="Save Data", command=app.save_to_excel).pack(side=LEFT, padx=2)
    ...
    # Metadata entry section for Location and Protocol
    meta_frame = ttk.Frame(app.control_panel)
    meta_frame.pack(fill=X, padx=5, pady=5)

    location_label = ttk.Label(meta_frame, text="Location:")
    location_label.grid(row=0, column=0, padx=2, sticky="w")
    app.location_entry = ttk.Entry(meta_frame, width=25)
    app.location_entry.grid(row=0, column=1, padx=2, sticky="w")

    protocol_label = ttk.Label(meta_frame, text="Protocol:")
    protocol_label.grid(row=1, column=0, padx=2, sticky="w")
    app.protocol_entry = ttk.Entry(meta_frame, width=25)
    app.protocol_entry.grid(row=1, column=1, padx=2, sticky="w")

    app.get_location = lambda: app.location_entry.get().strip()
    app.get_type = lambda: app.waveform_type_var.get().strip() if hasattr(app, "waveform_type_var") else ""
    app.get_protocol = lambda: app.protocol_entry.get().strip()
